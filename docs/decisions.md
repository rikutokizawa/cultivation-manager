...
- ダッシュボードのファーストビューには温度、湿度、CO2 濃度、接続状況、タンク水位、栽培画像 2 枚を集約表示する。
- Pi 移行前提の runtime は `sensor source` と `camera source` の切替式にする。
...
- dashboard の UI 改善は Figma を見本にして反映し、まずは dashboard から寄せる。
